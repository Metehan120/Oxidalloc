...
use crate::{
    ...
    inner::{
        ...
        free::{free_inner, validate_ptr_for_abi},
    },
    internals::{
...
use crate::{inner::fallback::realloc_fallback, internals::__errno_location, sys::NOMEM};

#[inline(always)]
pub unsafe fn realloc_inner(ptr: *mut c_void, new_size: size_t) -> *mut c_void {
    if ptr.is_null() {
        return alloc_inner(new_size);
    }
