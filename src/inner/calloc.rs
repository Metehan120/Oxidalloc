use crate::{
    HEADER_SIZE, OxHeader, OxidallocError, inner::alloc::alloc_inner,
    internals::hashmap::BIG_ALLOC_MAP, slab::SIZE_CLASSES,
};
...
unsafe fn calc_and_get(size: Layout, nmem: usize) -> Option<(*mut c_void, usize)> {
    ...
    let ptr = alloc_inner(effective_size);
    if ptr.is_null() {
        return None;
    }
...
#[inline(always)]
pub unsafe fn calloc_inner(nmemb: usize, size: usize) -> *mut c_void {
