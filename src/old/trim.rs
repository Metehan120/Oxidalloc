...
use crate::{
    FLAG_FREED, HEADER_SIZE, OX_CURRENT_STAMP, OxHeader, OxidallocError, TOTAL_ALLOCATED,
    global::{GLOBAL, GLOBAL_USAGE, GlobalHandler},
    internals::{SIZE_CLASSES, VA_MAP},
    malloc::current_thread_id,
        ...
        unsafe {
            for class in (0..cache.cache.len()).rev() {
                ...
                for _ in 0..cache.usages[class].load(Ordering::Relaxed) {
                    ...
                    let is_foreign = (*cache_mem).thread_id != current_thread_id();
                    let age_threshold = if is_foreign { 10 } else { 2 };
                ...
                while !to_trim.is_null() {
                    ...
                    if self.release_memory(to_trim, SIZE_CLASSES[class]) {
                        ...
                        GlobalHandler.push_to_global(class, to_trim, to_trim);
